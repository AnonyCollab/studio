...
export const Dashboard: React.FC<DashboardProps> = () => {
    const { theme, dashboardView, setDashboardView, tasks, currentUser, members } = useStore();
    ...
    const teamTasks = useMemo(() => {
        if (!currentUser?.teamName || !tasks || !members) return [];
        const teamMemberDisplayNames = members.filter(m => m.department === currentUser.teamName).map(m => m.displayName);
        return tasks.filter(t => 
            (t.assignee && t.assignee.displayName === currentUser.teamName) ||
    ...
    const renderTeamTab = () => {
        ...
        const workload = members.filter(m => m.type === 'user').map(member => ({
            name: member.displayName,
            count: teamTasks.filter(t => t.assignee.displayName === member.displayName && t.status !== 'Done').length,
        ...
        return (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-6">
                <div className="flex items-center justify-between mb-2">
                    <div>
                        <h2 className={`text-2xl font-bold ${textMain}`}>{currentUser.teamName} Pulse</h2>
