  ...
  const renderList = () => (
    <div className="max-h-[300px] overflow-y-auto custom-scrollbar p-2">
        ...
        {members.filter(a => a.type === 'user').map(assignee => (
            <button
                key={assignee.id}
                onClick={() => onSelect(assignee.displayName)}
            ...
            >
                ...
                <span className={`text-sm truncate ${textMain} ${currentAssignee === assignee.displayName ? 'font-bold' : ''}`}>{assignee.displayName}</span>
            </button>
        ))}
        ...
        {members.filter(a => a.type === 'team').map(team => (
            <button
                key={team.id}
                onClick={() => onSelect(team.displayName)}
                className={`w-full flex items-center gap-3 px-3 py-2 transition-colors rounded-lg ${itemHover}`}
            >
                ...
                <span className={`text-sm truncate ${textMain} ${currentAssignee === team.displayName ? 'font-bold' : ''}`}>{team.displayName}</span>
            </button>
        ))}
    </div>
  );
                    ...
                    <div className="space-y-2">
                        {members.filter(a => a.type === 'user').map(assignee => (
                            ...
                            >
                                ...
                                <span className="text-lg font-medium">{assignee.displayName}</span>
                                {currentAssignee === assignee.displayName && <div className="ml-auto w-2 h-2 rounded-full bg-brand-500" />}
                            </button>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="space-y-2">
                        {members.filter(a => a.type === 'team').map(team => (
                            ...
                            >
                                ...
                                <span className="text-lg font-medium">{team.displayName}</span>
                                {currentAssignee === team.displayName && <div className="ml-auto w-2 h-2 rounded-full bg-brand-500" />}
                            </button>
                        ))}
                    </div>
                </div>
